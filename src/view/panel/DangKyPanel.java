...
public class DangKyPanel extends BasePanel {
    ...
    public void addComp() {
        ...
        lbHienThi = createLabel("Đăng ký", 350, 20, f3, Color.RED, null);
        add(lbHienThi);
        lbDocGiaID = createLabel("ID_Độc Giả:", 250, 60, f2, Color.BLACK, null);
        add(lbDocGiaID);
        lbHoTen = createLabel("Họ tên:", 250, 90, f2, Color.BLACK, null);
        add(lbHoTen);
        lbSDT = createLabel("SĐT:", 250, 120, f2, Color.BLACK, null);
        add(lbSDT);
        lbTaiKhoan = createLabel("Tài khoản:", 250, 150, f2, Color.BLACK, null);
        add(lbTaiKhoan);
        lbMatKhau = createLabel("Mật khẩu:", 250, 180, f2, Color.BLACK, null);
        add(lbMatKhau);
        lbGioiTinh = createLabel("Giới Tính:", 250, 210, f2, Color.BLACK, null);
        add(lbGioiTinh);
        lbPhieuMuonID = createLabel("ID_phiếu mượn:", 250, 250, f2, Color.BLACK, null);
        add(lbPhieuMuonID);
        //
    ...
    protected void handleClick(String name) {
        if (name.equals(BT_DANG_KY)) {
            if (jtNguoiDungID.getText().equals("") || jtHoTen.getText().equals("") || jtSDT.getText().equals("")
                    || jtTaiKHoan.getText().equals("") || jpMatKhau.getPassword().equals("")
                ...
                if (checkAcc()) {
                    DocGia docGia = new DocGia(jtNguoiDungID.getText(), jtHoTen.getText(), jtSDT.getText(),
                            jtTaiKHoan.getText(), jpMatKhau.getText(), getGioiTinh, jtPhieuMuonID.getText());
                    acc.addListDocGia(docGia);
                    JOptionPane.showConfirmDialog(null, "TK bạn đọc đăng ký thành công", "thư viện",
                            JOptionPane.CLOSED_OPTION);
                    clearText();
                    acc.dangKyToDangNhap();
                }
            }
        }
    }
